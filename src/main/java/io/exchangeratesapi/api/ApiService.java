...
public class ApiService {
    ...
    public Mono<String> getAllRatesForBase(String base) {
        return request(base)
                .map(jsonNode -> jsonNode.get("conversion_rates").toString());
    }

    //Method to retrieve a specific exchange rate for a given base currency and target
    public Mono<String> getRate(String base, String target) {
        return request(base)
                ...
                .map(currencyCode -> {
                    ...
                    result.set("rate", currencyCode);
                    return result.toString();
                })
                .onErrorResume(throwable -> Mono.empty()); //Return empty Mono in case of any error
    }
}
